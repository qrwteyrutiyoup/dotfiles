...
{
    SOLARIZED_DIRCOLORS=~/.dircolors.solarized
    if [ -f "$SOLARIZED_DIRCOLORS" ]; then
        eval $(dircolors "$SOLARIZED_DIRCOLORS")
    else
        echo "Please install a dircolors file at [$SOLARIZED_DIRCOLORS]; ex.: https://raw.github.com/seebi/dircolors-solarized/master/dircolors.256dark"
    fi
